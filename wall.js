    ...
    return {
        init: function(rootNode) {
            ...
            gradientStart.setAttribute(
                "offset",
                (5 * NEAR_CLIPPING_PLANE) / GRADIENT_START + ""
            );
            ...
            gradientStop.setAttribute(
                "offset",
                (5 * NEAR_CLIPPING_PLANE) / GRADIENT_STOP + ""
            );
            ...
            for (var i = 0; i < NUM_SEGMENTS; i++) {
                ...
                mSegments[i].setAttribute(
                    "fill",
                    "url(#tunnel-wall-segment-fade-gradient)"
                );
        ...
        updateDOM: function(x, y, offset) {
            for (var i = 0; i < NUM_SEGMENTS; i++) {
                ...
                var x_ =
                    -x * Math.cos((segmentTheta * Math.PI) / 180) +
                    -y * Math.sin((segmentTheta * Math.PI) / 180);
                var y_ =
                    -x * Math.sin((segmentTheta * Math.PI) / 180) +
                    y * Math.cos((segmentTheta * Math.PI) / 180);

                var scale_x_ =
                    (0.1 *
                        MG.PROJECTION_PLANE_DISTANCE *
                        (1 - x_ / MG.TUNNEL_RADIUS)) /
                    (Math.tan((Math.PI * MG.FIELD_OF_VIEW) / 360.0) *
                        NEAR_CLIPPING_PLANE);
                var scale_y_ =
                    (0.1 * MG.PROJECTION_PLANE_DISTANCE) /
                    (Math.tan((Math.PI * MG.FIELD_OF_VIEW) / 360.0) *
                        NEAR_CLIPPING_PLANE);
