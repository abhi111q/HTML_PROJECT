...
AntiZoomAndPan.prototype.update = function() {
  if (this.svgNodes.length > 0) {
    ...
    for (var i = 0; i < this.svgNodes.length; i++) {
      ...
      var transform =
        "matrix(" + [CTM.a, CTM.b, CTM.c, CTM.d, CTM.e, CTM.f].join(",") + ")";
...
ViewBox.prototype.getTM = function() {
  ...
  if (this.alignX == "none") {
    ...
    if (
      (x_ratio < y_ratio && this.meetOrSlice == "meet") ||
      (x_ratio > y_ratio && this.meetOrSlice == "slice")
    ) {
      var x_trans = 0;
      var x_diff = windowWidth * y_ratio - this.width;
      if (this.alignX == "Mid") x_trans = -x_diff / 2;
      else if (this.alignX == "Max") x_trans = -x_diff;
      ...
      if (this.alignY == "Mid") y_trans = -y_diff / 2;
      else if (this.alignY == "Max") y_trans = -y_diff;
          ...
          if (0 <= xRoot && xRoot <= 1) {
            for (var k = 0; k < yRoots.length; k++) {
              if (Math.abs(xRoot - yRoots[k]) < TOLERANCE) {
                result.points.push(
                  c22.multiply(s * s).add(c21.multiply(s).add(c20))
                );
                break checkRoots;
              }
            }
          ...
          for (var k = 0; k < yRoots.length; k++) {
            if (Math.abs(xRoot - yRoots[k]) < TOLERANCE) {
              result.points.push(
                c23
                  .multiply(s * s * s)
                  .add(c22.multiply(s * s).add(c21.multiply(s).add(c20)))
              );
              break checkRoots;
            }
          }
...
Intersection.intersectBezier2Ellipse = function(p1, p2, p3, ec, rx, ry) {
  ...
  var roots = new Polynomial(
    ...
    ryry * (2 * c2.x * c0.x + c1.x * c1.x) +
      rxrx * (2 * c2.y * c0.y + c1.y * c1.y) -
      2 * (ryry * ec.x * c2.x + rxrx * ec.y * c2.y),
    2 * (ryry * c1.x * (c0.x - ec.x) + rxrx * c1.y * (c0.y - ec.y)),
    ryry * (c0.x * c0.x + ec.x * ec.x) +
...
Intersection.intersectBezier3Bezier3 = function(
  a1,
  a2,
  a3,
  a4,
...
Intersection.intersectBezier3Ellipse = function(p1, p2, p3, p4, ec, rx, ry) {
  ...
  var poly = new Polynomial(
    ...
    2 * (c3.x * c1.x * ryry + c3.y * c1.y * rxrx) +
      c2.x * c2.x * ryry +
      c2.y * c2.y * rxrx,
    2 * c3.x * ryry * (c0.x - ec.x) +
      2 * c3.y * rxrx * (c0.y - ec.y) +
      2 * (c2.x * c1.x * ryry + c2.y * c1.y * rxrx),
    2 * c2.x * ryry * (c0.x - ec.x) +
...
Intersection.intersectBezier3Rectangle = function(p1, p2, p3, p4, r1, r2) {
  ...
  var inter3 = Intersection.intersectBezier3Line(
    p1,
    p2,
    p3,
    p4,
  ...
  var inter4 = Intersection.intersectBezier3Line(
    p1,
    p2,
    p3,
    p4,
    ...
    if (index + param_length < tokens.length) {
      for (var i = index; i < index + param_length; i++) {
        ...
        if (number.typeis(Path.NUMBER)) params[params.length] = number.text;
          ...
          throw new Error(
            "Parameter type is not a number: " + mode + "," + number.text
          );
      }
      var segment;
...
AbsolutePathSegment.prototype.init = function(
  command,
  params,
  owner,
  previous
...
RelativePathSegment.prototype.init = function(
  command,
  params,
  owner,
  previous
